...
class MotorTest(unittest.TestCase):
    def setup(self):
        ...
        rospy.wait_for_service('/motor_on', Trigger)
    ...
    def test_put_cmd_vel(self):
        ...
        self.file_check("rtmotor_raw_l0" ,0, "don't stip after 1[s]")

    def test_on_off(self):
        off = rospy.ServiceProxy('/motor_off', Trigger)
        ret = off()
        self.assertEqual(ret.success, True, "motor off dese not successed")
        ...
        with open("/dev/rtmotoren0", "r") as f:
            data = f.readline()
            self.assertEqual(data, "1\n", "wrong value in rtmotor0 at mnotor on")

    def test_put_value_timed(self):
        tm = rospy.ServicePrxy('/timed_motion', TimedMotion)
        tm(-321, 654, 1500)
        with open("/dev/rtmotor0", "r") as f:
            data = f.readline()
            self.assertEqual(data, "-321 654 1500\n","value does not written to rt motion0")
